...
export const importComponent = async (element, classID) => {
  if (element.length && !element.hasClass("init")) {
    try {
      ...
      const resolvedModulePath = Object.keys(modules).find(path => path.includes(`${classID}.js`));
      ...
      if (resolvedModulePath) {
        ...
        // console.error(`Component ${classID} not found in ./components/`);
      }
    } catch (error) {
      // console.error(`Error importing component ${classID}:`, error);
    }
  }
};
